...
results_frame = pd.DataFrame(columns=['Enemy', 'Fitness', 'Player life', 'Enemy life', 'Time', 'Island'])

file = 'EC_assignment1_part1/enemy3_4islands_run1.txt'

with open(file) as f:
    numbers = r"[-+]?\d*\.\d+|\d+"
    for line in f:
        if line.startswith('Island:'):
            island = re.findall(numbers, line)[0]
        if line.startswith('RUN:'):
            results = re.findall(numbers, line)
            results = [float(i) for i in results]
            results.append(int(island))
...
# plt.fill_between(analysis_frame.index, analysis_frame["ub"], analysis_frame["lb"], facecolor='red', alpha=0.5,
#                  interpolate=True)
